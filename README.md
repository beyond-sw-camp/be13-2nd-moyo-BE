...
|   홍도현 	|   처음으로 프론트엔드와 백엔드를 각각 분리된 프로젝트로 구성하고 REST API를 통해 연동해보며, 하나의 서비스를 동작하게 만드는 과정의 복잡함과 중요성을 느꼈습니다. 데이터를 주고받는 과정에서 발생한 불일치 문제를 해결하며, 백엔드 개발자로서 어떤 데이터를 어떻게 전달할지에 대한 명확한 API 명세가 얼마나 중요한지 깨달았고, 프론트 개발을 직접 경험해보며 프론트와 어떻게 소통하고 협업해야 할지 배울 수 있었습니다. 저에게 정말 큰 배움이 된 프로젝트였고, 함께한 모든 팀원들께도 진심으로 감사드립니다!	|
|   김민석  	|   	|
